...
class TableItemService
{
     ...
     * @throws Exception
     */
    public function list(PaginateRequest $request, DiningTable $table)
    {
        try {
            ...
            return Item::withoutGlobalScope(BranchScope::class)->with('media', 'category', 'tax')
    ...
    {
        try {
            return Item::withoutGlobalScope(BranchScope::class)->where(['is_featured' => Ask::YES, 'status' => Status::ACTIVE, 'branch_id' => $request->branch_id])->inRandomOrder()->limit(8)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    ...
    {
        try {
            return Item::withoutGlobalScope(BranchScope::class)->withCount('orders')->where(['status' => Status::ACTIVE, 'branch_id' => $request->branch_id])->orderBy('orders_count', 'desc')->limit(6)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
