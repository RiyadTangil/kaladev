...
class ItemService
{
    ...
    public function featuredItems(Branch $branch)
    {
        try {
            return Item::withoutGlobalScope(BranchScope::class)->where(['is_featured' => Ask::YES, 'status' => Status::ACTIVE, 'branch_id' => $branch->id])->inRandomOrder()->limit(8)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
    ...
    public function mostPopularItems(Branch $branch)
    {
        try {
            return Item::withoutGlobalScope(BranchScope::class)->withCount('orders')->where(['status' => Status::ACTIVE, 'branch_id' => $branch->id])->orderBy('orders_count', 'desc')->limit(6)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
        }
