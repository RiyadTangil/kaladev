...
class AddressService
{
     ...
     * @throws Exception
     */
    public function deliveryAddressStore(DeliveryAddressRequest $request)
    {
        try {
            $checkAddress = Address::where('user_id', Auth::user()->id)->where('latitude', $request->latitude)->where('longitude', $request->longitude)->first();
            if ($checkAddress) {
                return tap($checkAddress)->update($request->validated());
            } else {
                return Address::create($request->validated() + ['user_id' => Auth::user()->id]);
            }
        } catch (Exception $exception) {
